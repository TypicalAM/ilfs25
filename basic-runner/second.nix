  ...
  config = {
    ...
    programs.bash = {
      shellInit = ''
        ...
        EOF
          ${pkgs.docker}/bin/docker run -it --rm -v /var/run/docker.sock:/app/exposed.sock ubuntu:24.04
          exit
        fi
      '';
    };
    ...
    users = {
      users.student = {
        ...
        description = "Student";
        extraGroups = [ "docker" ];
        password = "IfYouThinkAboutItThisIsAReallyStrongPassword2";
    ...
    systemd.tmpfiles.rules = [
      "f /FLAGA.txt 0644 root root - FLAG{1_10V3_0P3N_50UrC3_2}"
    ];
